  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#f8f8f6" }}>
      <div style={{ background: "#fff", padding: "2.5rem", borderRadius: "16px", boxShadow: "0 2px 16px #0001", width: "360px" }}>
        ...
        {error && <div style={{ background: "#fee", color: "#c33", padding: "0.75rem", borderRadius: "8px", marginBottom: "1rem", fontSize: "0.875rem" }}>{error}</div>}
    ...
    <div style={{ background: "#fff", padding: "1.5rem", borderRadius: "12px", border: "1px solid #e8e8e4" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
        <div>
          ...
          <input style={inputStyle} value={form.title} onChange={e => setField("title", e.target.value)} placeholder="e.g. Jeera Cola 500ml" />
        </div>
        <div>
          <label style={labelStyle}>Category *</label>
          <input style={inputStyle} value={form.category} onChange={e => setField("category", e.target.value)} placeholder="e.g. Energy Drink" />
        </div>
      </div>

      <label style={{ ...labelStyle, marginTop: "0.75rem" }}>Description *</label>
      <textarea style={{ ...inputStyle, minHeight: "80px", resize: "vertical" }} value={form.description} onChange={e => setField("description", e.target.value)} placeholder="Product description..." />

      <label style={{ ...labelStyle, marginTop: "0.75rem" }}>Image {initial ? "(optional — nahi badlenge toh purani rahegi)" : "*"}</label>
      <div style={{ display: "flex", alignItems: "center", gap: "1rem", marginTop: "0.25rem" }}>
        {preview && <img src={preview} alt="preview" style={{ width: 80, height: 80, objectFit: "cover", borderRadius: "8px", border: "1px solid #e8e8e4" }} />}
        <button style={{ ...btnStyle, background: "#f0f0ec", color: "#333" }} onClick={() => fileRef.current.click()}>
      ...
      <div style={{ marginTop: "1rem" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.5rem" }}>
          ...
          <button style={{ ...btnStyle, fontSize: "0.8rem", padding: "0.3rem 0.75rem" }} onClick={addVariation}>+ Add Size</button>
        </div>
        {form.priceVariations.map((pv, i) => (
          <div key={i} style={{ display: "flex", gap: "0.75rem", marginBottom: "0.5rem", alignItems: "center" }}>
            <select style={{ ...inputStyle, flex: 2 }} value={pv.size} onChange={e => updateVariation(i, "size", e.target.value)}>
              {VALID_SIZES.map(s => <option key={s} value={s}>{s}</option>)}
            </select>
            <input style={{ ...inputStyle, flex: 1 }} type="number" min="0" value={pv.price} onChange={e => updateVariation(i, "price", e.target.value)} placeholder="₹ Price" />
            {form.priceVariations.length > 1 && (
              <button onClick={() => removeVariation(i)} style={{ background: "#fee", color: "#c33", border: "none", borderRadius: "6px", padding: "0.4rem 0.6rem", cursor: "pointer", fontSize: "1rem" }}>×</button>
            )}
          </div>
        ))}
      </div>
...
const labelStyle = { display: "block", fontSize: "0.8rem", fontWeight: 500, color: "#555", marginBottom: "0.25rem" };
const btnStyle = {
...
const thStyle = { padding: "0.75rem 1rem", textAlign: "left", fontSize: "0.8rem", fontWeight: 600, color: "#555" };
